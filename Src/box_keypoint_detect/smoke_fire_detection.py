...
def main():   
    ...
    if not cap.isOpened():
        logger.log_error("Could not open camera stream.")
        return
    logger.log_info( "Camera stream is active and running.")
    while True:
    #for file in files:
        ...
        ret, frame = cap.read()
        if frame is None:
            logger.log_warning( f"Frame is None")
            continue
        logger.log_info( "Frame is loaded")
        result = smoke_model.predict(frame)
        logger.log_debug( f"Size of result: {len(result.boxes)}")
        boxes = result.boxes
        names = result.names
        for i, name in enumerate(names):
            logger.log_debug( f"List class: {names[i]}")
            logger.log_debug( f"List class: {name}")
        for box in boxes:
