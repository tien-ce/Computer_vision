...
class HumanDetector:
    ...
    def detect_human(self, frame) -> list[BoundingBox]:
        ...
        result = self.human_detect_model.predict(frame=frame)
        boxes = result.boxes
        ...
        for i,box in enumerate(boxes):
            ...
            if class_id != 0: 
                continue # If it is not person, ignore
            ...
            self.logger.log_debug(f"Lenght detected human: {detected_human}")
            ...
            # Get frame dimensions to prevent out-of-bounds slicing
                ...
                cv2.imshow(f"Cropped human {i}", resized_frame)
