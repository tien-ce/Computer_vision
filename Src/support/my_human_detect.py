...
from my_dataclass import BoundingBox
...
INTERNAL_MODEL_PATH = (MODULE_DIR / ".." / ".." / "Models" / "yolov8x.pt").resolve()
...
class HumanDetector:
    ...
    def track_human(self, frame, device = "cpu") -> list[BoundingBox]:
        ...
        if frame is None:
         ...
         # Run the tracking pipeline on the current frame
        # persist=True: maintains identity state across consecutive frames
        # tracker="bytetrack.yaml": uses ByteTrack for efficient state association
        # classes=[0]: filters the detection to only track 'Person'
        results = self.human_detect_model.track(source=frame, persist=True, tracker="bytetrack.yaml",
                                                 classes=[0], imgsz=1088, verbose = False, device = device)
        result = results[0]
        boxes = result.boxes
        detected_human = []
        for i,box in enumerate(boxes):
            ...
            # ---------------------------------------------------------------------
            bbox = BoundingBox(x1=x1, y1=y1, x2=x2, y2=y2, class_id=0, confidence=confidence, track_id=track_id)
            detected_human.append(bbox)
            self.logger.log_debug(f"Length detected human: {detected_human}")
            # Context-aware drawing inside the module
        return detected_human

    def detect_human(self, frame, device = "cpu") -> list[BoundingBox]:
        """
        Processes a single frame matrix, runs object detection, and extracts person coordinates.
        ...
        if frame is None:
         ...
         # Run the tracking pipeline on the current frame
        results = self.human_detect_model.predict(source=frame, classes=[0],
                                                  imgsz=1088, verbose = False, device = device)
        result = results[0]
        boxes = result.boxes
        detected_human = []
        for i,box in enumerate(boxes):
            ...
            self.logger.log_debug(f"Coordinates - x1: {type(x1)}={x1}, y1: {type(y1)}={y1}, x2: {type(x2)}={x2}, y2: {type(y2)}={y2}")
            # ---------------------------------------------------------------------
            bbox = BoundingBox(x1=x1, y1=y1, x2=x2, y2=y2, class_id=0, confidence=confidence)
            detected_human.append(bbox)
            self.logger.log_debug(f"Length detected human: {detected_human}")
            # Context-aware drawing inside the module
        return detected_human
