...
class UniformDetector:
    def __init__(self, log_level: LogLevel = LogLevel.INFO,confidence: float = 0.5, draw: bool = False):
        ...
        self.shirt_detect_model = ModelInference(str(INTERNAL_MODEL_PATH))
    ...
    def detect_shirt(self, frame) -> list[BoundingBox]:
        """
        ...
        result = self.shirt_detect_model.predict(frame=frame)
        boxes = result.boxes
        num_shirt = len(boxes)
        self.logger.log_debug(f"Number of detectd shirt: {num_shirt}")
        ...
        for i,box in enumerate(boxes):
            ...
            detected_shirt.append(bbox)
            self.logger.log_debug(f"Lenght detected shirt: {detected_shirt}")
            ...
            # Get frame dimensions to prevent out-of-bounds slicing
                ...
                cv2.imshow(f"Cropped shirt {i}", resized_frame)
        return detected_shirt
